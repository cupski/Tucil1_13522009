...
class InputHandler:
    ...
    def manual(self):
        ...
        matrix_size = input("Masukkan ukuran matriks (jumlah kolom dan baris dipisahkan oleh spasi): ")
        num_column, num_row = map(int, matrix_size.split())
        if(buffer_size > (num_column*num_row)):
            print("Buffer Size Tidak boleh lebih besar dari ukuran Matrix")
            self.continue_menu()
        elif (num_column < 2 or num_row < 2):
            print("Baris dan kolom harus lebih dari pada satu")
            self.continue_menu()
        else:
